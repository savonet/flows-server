      ...
      match path with
        | "/" -> (
            match meth with
              ...
              | `GET ->
                  let h =
                    HTML.create ~css:"flows.css" ~title:"Liquidsoap radios" ()
                  in
...
|};
                  ...
                  List.iter
                    (fun (_, (r : Radio.t)) ->
                      ...
                      in
                      let stream =
                        if r.streams = [] then "" else (List.hd r.streams).url
                      in
                      HTML.w ~nl:false h
                        {|
<div class="radio">
                           ...
                           List.map
                             (fun s ->
                               Printf.sprintf "📻 <a href=\"%s\">%s</a>"
                                 s.Radio.url s.format)
                             r.Radio.streams
                           |> String.concat ", "
                         in
                         Printf.sprintf
                           "<a href=\"%s\">%s</a>: %s (%s)<br/>▶ <em>%s</em> \
                            by %s"
                           r.Radio.website r.name r.description streams r.title
                           r.artist)
                       radios);
                  (* Map *)
                  HTML.h2 h "The map";
